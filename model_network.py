...
class ST_LSTM(nn.Module):
    def __init__(self, embedding_size, hidden_size, num_layers, dropout_rate, device):
        ...
        self.Wt_q = nn.Linear(embedding_size//2, embedding_size//2, bias=True)
        ...
        self.layer_norm_s = nn.LayerNorm(embedding_size // 2, eps=1e-6)
    ...
    def forward(self, att_s, att_t, packed_input):
        ...
        s_k = self.Ws_k(att_s)
        s_v = self.Ws_v(att_s)
        attn = torch.matmul(s_q / self.temperature, s_k.transpose(1, 2))
        attn = F.softmax(attn, dim=-1)
        ...
        _, seq_lengths = pad_packed_sequence(packed_input, batch_first=True)
        ...
        # Attention...
        # (batch_size, seq_len, 2 * num_hiddens)
        u = torch.tanh(torch.matmul(paded_output, self.w_omega))
        # (batch_size, seq_len)
        ...
        # (batch_size, seq_len, 2 * num_hiddens)
        scored_outputs = paded_output * att_score
...
class ST_Encoder(nn.Module):
    ...
    def forward(self, network, traj_seqs, time_seqs):
        ...
        att_output = self.encoder_ST(att_s, att_t, packed_input)
